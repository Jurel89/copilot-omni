...
1. **PRD Setup** (first iteration only):
   a. Check if `prd.json` exists (in project root or `.omni/`). If it already exists, read it and proceed to Step 2.
   b. If no `prd.json` exists, the system has auto-generated a scaffold. Read `.omni/prd.json`.
...
8. **On approval**: After Step 7.6 passes (with Step 7.5 completed, or skipped via `--no-deslop`), run `/copilot-omni:cancel` to cleanly exit and clean up all state files
...
- Stop when the user says "stop", "cancel", or "abort" -- run `/copilot-omni:cancel`
...
- [ ] `/copilot-omni:cancel` run for clean state cleanup
</Final_Checklist>
