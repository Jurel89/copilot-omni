...
Configure the copilot-omni HUD (Heads-Up Display) for the statusline.
...
| Command | Description |
|---------|-------------|
| `/copilot-omni:hud` | Show current HUD status (auto-setup if needed) |
| `/copilot-omni:hud setup` | Install/repair HUD statusline |
| `/copilot-omni:hud minimal` | Switch to minimal display |
| `/copilot-omni:hud focused` | Switch to focused display (default) |
| `/copilot-omni:hud full` | Switch to full display |
| `/copilot-omni:hud status` | Show detailed HUD status |

## Auto-Setup

When you run `/copilot-omni:hud` or `/copilot-omni:hud setup`, the system will automatically:
1. Check if `~/.claude/hud/omni-hud.mjs` exists
...
**IMPORTANT**: If the argument is `setup` OR if the HUD script doesn't exist at `~/.claude/hud/omni-hud.mjs`, you MUST create the HUD files directly using the instructions below.

### Setup Instructions (Run These Commands)

**Step 1:** Check if setup is needed:
```bash
node -e "const p=require('path'),f=require('fs'),d=process.env.CLAUDE_CONFIG_DIR||p.join(require('os').homedir(),'.claude');console.log(f.existsSync(p.join(d,'hud','omni-hud.mjs'))?'EXISTS':'MISSING')"
```

**Step 2:** Verify the plugin is installed:
```bash
node -e "const p=require('path'),f=require('fs'),d=process.env.CLAUDE_CONFIG_DIR||p.join(require('os').homedir(),'.claude'),b=p.join(d,'plugins','cache','omc','copilot-omni');try{const v=f.readdirSync(b).filter(x=>/^\d/.test(x)).sort((a,c)=>a.localeCompare(c,void 0,{numeric:true}));if(v.length===0){console.log('Plugin not installed - run: /plugin install copilot-omni');process.exit()}const l=v[v.length-1],h=p.join(b,l,'dist','hud','index.js');console.log('Version:',l);console.log(f.existsSync(h)?'READY':'NOT_FOUND - try reinstalling: /plugin install copilot-omni')}catch{console.log('Plugin not installed - run: /plugin install copilot-omni')}"
```

**Step 3:** If omni-hud.mjs is MISSING or argument is `setup`, install the HUD wrapper and its dependency from the canonical template:

```bash
HUD_DIR="${CLAUDE_CONFIG_DIR:-$HOME/.claude}/hud"
mkdir -p "$HUD_DIR/lib"
cp "${CLAUDE_PLUGIN_ROOT}/scripts/lib/hud-wrapper-template.txt" "$HUD_DIR/omni-hud.mjs"
...
```bash
node -e "if(process.platform==='win32'){console.log('Skipped (Windows)')}else{require('fs').chmodSync(require('path').join(process.env.CLAUDE_CONFIG_DIR||require('path').join(require('os').homedir(),'.claude'),'hud','omni-hud.mjs'),0o755);console.log('Done')}"
```
...
```bash
node -e "const p=require('path').join(require('os').homedir(),'.claude','hud','omni-hud.mjs').split(require('path').sep).join('/');console.log(JSON.stringify(p))"
```
...
```json
{
  "statusLine": {
    "type": "command",
    "command": "node ${CLAUDE_CONFIG_DIR:-$HOME/.claude}/hud/omni-hud.mjs"
  }
}
```
...
```json
{
  "statusLine": {
    "type": "command",
    "command": "node C:/Users/username/.claude/hud/omni-hud.mjs"
  }
}
```
...
```bash
node -e "const p=require('path'),f=require('fs'),d=process.env.CLAUDE_CONFIG_DIR||p.join(require('os').homedir(),'.claude'),t=p.join(d,'hud','omni-hud.js');try{if(f.existsSync(t)){f.unlinkSync(t);console.log('Removed legacy omni-hud.js')}else{console.log('No legacy script found')}}catch{}"
```
...
```
[copilot-omni] ralph | ultrawork | todos:2/5
```

### Focused (Default)
Shows all relevant elements:
```
[copilot-omni] branch:main | ralph:3/10 | US-002 | ultrawork skill:planner | ctx:67% | agents:2 | bg:3/5 | todos:2/5
```

### Full
Shows everything including multi-line agent details:
```
[copilot-omni] repo:copilot-omni branch:main | ralph:3/10 | US-002 (2/5) | ultrawork | ctx:[████░░]67% | agents:3 | bg:3/5 | todos:2/5
...
Legacy config location (deprecated): `~/.claude/.omni/hud-config.json`
...
1. Run `/copilot-omni:hud setup` to auto-install and configure
2. Restart Claude Code after setup completes
3. If still not working, run `/copilot-omni:omni-doctor` for full diagnostics

**Legacy string format migration:** Older copilot-omni versions wrote `statusLine` as a plain string (e.g., `"~/.claude/hud/omni-hud.mjs"`). Modern Claude Code (v2.1+) requires an object format. Running the installer or `/copilot-omni:hud setup` will auto-migrate legacy strings to the correct object format:
```json
{
  "statusLine": {
    "type": "command",
...
**Node 24+ compatibility:** The HUD wrapper script imports `homedir` from `node:os` (not `node:path`). If you encounter `SyntaxError: The requested module 'path' does not provide an export named 'homedir'`, re-run the installer to regenerate `omni-hud.mjs`.

Manual verification:
- HUD script: `~/.claude/hud/omni-hud.mjs`
