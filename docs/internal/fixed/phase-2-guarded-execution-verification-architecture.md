# Guarded Execution and Verification Engine — Architecture
...
- Task execution journal entries with policy decisions and command logs
- Verification report schema
- Protected-path and allowlist policy files
- Prompt-injection scan result records
...
- Path traversal validation for all file-system inputs.
- Shell argument validation and command normalization before allowlist matching.
- Protected-path editing only through explicit policy or run-level exception artifacts.
...
Phase 2 creates the safe execution substrate. Phase 3 can then persist and retrieve knowledge about those runs without guessing from unstructured chat alone.
