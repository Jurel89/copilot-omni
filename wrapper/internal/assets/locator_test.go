...
func TestResolveFromExecutableSourceLayout(t *testing.T) {
	...
	execPath := createExecutable(t, filepath.Join(repoRoot, "wrapper", "omni"))
...
func TestResolveFromExecutableInstalledLayout(t *testing.T) {
	...
	execPath := createExecutable(t, filepath.Join(prefix, "bin", "omni"))
...
func TestResolveFromExecutableEnvOverride(t *testing.T) {
	assetRoot := t.TempDir()
	createAssetRoot(t, assetRoot)
	customExecPath := createExecutable(t, filepath.Join(t.TempDir(), "custom", "omni"))
